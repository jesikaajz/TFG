...
from ..schemas.academic_year_schema import (
    AcademicYearCreate,
    AcademicYearPatch,
...
def create(year: AcademicYearCreate, db: Session = Depends(get_db), _: User = Depends(admin_required)):
...
def update(year_id: int, data: AcademicYearCreate, db: Session = Depends(get_db), _: User = Depends(admin_required)):
    year = db.query(AcademicYear).filter(AcademicYear.id == year_id).first()
    if not year:
        raise HTTPException(404, "Academic year not found")
...
def patch(year_id: int, data: AcademicYearPatch, db: Session = Depends(get_db), _: User = Depends(admin_required)):
    year = db.query(AcademicYear).filter(AcademicYear.id == year_id).first()
    if not year:
        raise HTTPException(404, "Academic year not found")
...
def delete(year_id: int, db: Session = Depends(get_db), _: User = Depends(admin_required)):
    year = db.query(AcademicYear).filter(AcademicYear.id == year_id).first()
    if not year:
        raise HTTPException(404, "Academic year not found")
